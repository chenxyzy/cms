...
<title>论坛风格模板列表</title>
</head>
<s:action name="admin_bbs_style_query">
...
<div class="aebox">
 <div class="aeheadbox">
  ...
  <div class="aeheadm">>　论坛风格模板列表</div>
 ...
 <div class="aelist">
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
   ...
   <tr class="trlist">
    ...
    <td class="list_6">显示状态</td>
    <td class="list_7">操作</td>
   </tr>

   <s:iterator value="#request.bbsStyleAll" id="x" status="st">      
   <tr >
    <td class="list_1">${st.index+1}</td>
    <td class="list_2">${x.styleName} </td>
    <td class="list_3">${x.author}</td>
    <td class="list_4">${x.description}</td>
    <td class="list_5"><s:if test="def"><b><img src="./images/ok.gif" /> 当前默认</b></s:if><s:else><font color="#FF0000"><a href="admin_bbs_style_setDefault.action?id=${x.id}">设为默认</a></font></s:else></td>
    <td class="list_6"><s:if test="state">显示</s:if><s:else><font color="#FF0000">隐藏</font></s:else></td>
    <td class="list_7"><a href="admin_bbs_style_del.action?id=${x.id}" onclick="{if(confirm('确定删除风格模板“${x.styleName}”吗?\n该删除将不可撤消！')){return true;}return false;}">删除</a><s:if test="state"> <a href="admin_bbs_style_changeState.action?state=false&id=${x.id}">隐藏</a> </s:if><s:else> <a href="admin_bbs_style_changeState.action?state=true&id=${x.id}">显示</a></s:else> <a href="javascript:showStyleCopyDiv('admin_bbs_style_copy.jsp?id=${x.id}');">复制</a> <a href="admin_bbs_style_getMetaDataByID.action?styleID=${x.id}&table=bbs_style">编辑</a></td>

   </tr>
   
</s:iterator>
  </table>
  <div class="list_oneline2">共有<s:property value="#request.bbsStyleAll.size()"/>&nbsp;个风格模板</div>
   </div>
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
  <form method="POST" action="admin_bbs_style_add.action">
  <tr >
    <td class="list_7">风格模板名：</td>
    <td class="list_8"><input type="text" name="styleName" size="60"></td>
  </tr>
  <tr >
    <td class="list_7">作者或版权信息：</td>
    <td class="list_8"><input type="text" name="author" size="60"></td>
  </tr>
  <tr >
    <td class="list_7">描述：</td>
    <td class="list_8"><input type="text" name="description" size="60"></td>
  </tr>
...
<center><br><a href="javascript:reload();">刷新</a> <a href="admin_bbs_style_list.jsp?mod=1">显示所有</a> <a href="admin_bbs_style_list.jsp?mod=0">默认显示</a></center>
</div>
</div>
<div class="aebox">
   <div class="aeheadbox">
 ...
 <div class="aelist">
 <form name="upload" action="admin_bbs_style_importFromFile.action" method="post" enctype="multipart/form-data">
