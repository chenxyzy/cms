...
<div class="aebox">
 <div class="aeheadbox">
  ...
  <div class="aeheadm">>　用户代码分类列表</div>
  <div class="aeheadr"></div>
 </div>
 <div class="aelist">
<s:action name="admin_custom_code_category_query" />
  <table width="100%" border="0" cellpadding="0" cellspacing="0">

       
   <tr class="trlist">
    <td class="list_1">序号</td>
    <td class="list_2">分类id号</td>
    <td class="list_3">分类名称</td>
    <td class="list_4">操作</td>
   </tr>

   <s:iterator value="#request.categoryAll" var="its" id="x" status="st">      
   <tr >
    <td class="list_1">${st.index+1}</td>
    <td class="list_2">${x.id}</td>
    <td class="list_3"><a href="admin_custom_code_list.jsp?gid=${x.id}">${x.name}</a> </td>
    <td class="list_4"><a href="admin_custom_code_category_del.action?id=${x.id}" onclick="{if(confirm('确定删除代码类型“${x.name}”吗?\n该删除将不可撤消！')){return true;}return false;}">删除</a>  <a href="admin_custom_code_category_findById.action?id=${x.id}">修改</a></td>
   </tr>
   
</s:iterator>
  
  </table>
   <div class="list_oneline2">共有：<s:property value="#request.categoryAll.size()"/> &nbsp;个代码类型</div>
  ...
  <div class="aebox">
   <div class="aeheadbox">
  <div class="aeheadl"></div>
  <div class="aeheadm">>　增加代码分类</div>
  <div class="aeheadr"></div>
 </div>
  <div class="aelist">
  <s:property value="actionErrors[0]"/>
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
  <form method="POST" action="admin_custom_code_category_add.action">
  <tr >
    <td class="list_7">类型名称：</td>
    <td class="list_8"><input type="text" autocomplete="off" name="cc.name" size="60"></td>
  </tr>
