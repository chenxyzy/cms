...
<title>投票系统风格模板列表</title>
</head>
<s:action name="admin_vote_style_query">
...
<div class="aebox">
 <div class="aeheadbox">
  ...
  <div class="aeheadm">>　投票系统风格模板列表</div>
 ...
 <div class="aelist">
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
   ...
   <tr >
    ...
    <td class="list_2">${x.title} </td>
    <td class="list_3">${x.author}</td>
    <td class="list_4">${x.description}</td>
    <td class="list_5"><s:if test="state"><b><img src="./images/ok.gif" /> 当前默认</b></s:if><s:else><font color="#FF0000"><a href="admin_vote_style_setDefault.action?id=${x.id}">设为默认</a></font></s:else></td>
    
    <td class="list_6"><a href="admin_vote_style_del.action?id=${x.id}" onclick="{if(confirm('确定删除风格模板“${x.title}”吗?\n该删除将不可撤消！')){return true;}return false;}">删除</a> <a href="javascript:showStyleCopyDiv('admin_vote_style_copy.jsp?id=${x.id}');">复制</a> <a href="admin_vote_style_getMetaDataByID.action?styleID=${x.id}&table=vote_style">编辑</a></td>

   </tr>
</s:iterator>
  </table>
  <div class="list_oneline2">共有<s:property value="#request.voteStyleAll.size()"/>&nbsp;个风格模板</div>
   </div>
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
  <form method="POST" action="admin_vote_style_add.action">
  <tr >
    <td class="list_7">风格模板名：</td>
    <td class="list_8"><input type="text" name="vs.title" size="60"></td>
  </tr>
  <tr >
    <td class="list_7">作者或版权信息：</td>
    <td class="list_8"><input type="text" name="vs.author" size="60"></td>
  </tr>
  <tr >
    <td class="list_7">描述：</td>
    <td class="list_8"><input type="text" name="vs.description" size="60"></td>
  </tr>
...
<div class="aebox">
 ...
 <div class="aelist">
 <form name="upload" action="admin_vote_style_importFromFile.action" method="post" enctype="multipart/form-data">
