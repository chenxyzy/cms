 ...
 <div class="aelist">
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
   ...
   <tr class="trlist">
    ...
    <td class="list_5">建立时间</td>
    <td class="list_6">操作</td>
   </tr>
<s:action name="admin_integral_rule_query"></s:action>
   <s:iterator value="integralRuleAll" id="x" status="st">      
   <tr >
    <td class="list_1">${st.index+1}</td>
    <td class="list_2">${x.id} </td>
    <td class="list_2">${x.name} </td>
    <td class="list_4"><s:if test="state"><b>当前默认</b> <a href="admin_integral_rule_changeState.action?id=${x.id}&state=false&localPostion=${localPostion}">取消</a></s:if><s:else><font color="#FF0000"><a href="admin_integral_rule_changeState.action?id=${x.id}&state=true&localPostion=${localPostion}">设为默认</a></font></s:else></td>
    <td class="list_3">${x.createTime}</td>
    <td class="list_6"><a href="admin_integral_rule_del.action?id=${x.id}&localPostion=${localPostion}" onclick="{if(confirm('确定删除积分方案“${x.name}”吗?\n该删除将不可撤消！')){return true;}return false;}">删除</a>  <a href="admin_integral_rule_findById.action?id=${x.id}&localPostion=${localPostion}">修改</a></td>

   </tr>
</s:iterator>
  </table>
  <div class="list_oneline2">共有<s:property value="#request.integralRuleAll.size()"/>&nbsp;个积分方案</div>
   </div>

  <div class="aebox">
   <div class="aeheadbox">
  <div class="aeheadl"></div>
  <div class="aeheadm">>　增加积分方案</div>
  <div class="aeheadr"></div>
 </div>
  <div class="aelist">
  <s:property value="actionErrors[0]" />
  <form method="POST" action="admin_integral_rule_add.action?localPostion=${localPostion}">
  
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
  
  <tr >
    <td class="list_7">名称：</td>
    <td class="list_8"><input type="text" autocomplete="off" name="ir.name" size="60"></td>
  </tr>
  <tr >
    <td class="list_7">发布得分：</td>
    <td class="list_8"><input type="text" name="ir.valueOfAdd" autocomplete="off" size="60"></td>
  </tr>
  <tr >
    <td class="list_7">删除扣分：</td>
    <td class="list_8"><input type="text" name="ir.valueOfDel" autocomplete="off" size="60"></td>
  </tr>
  <tr >
    <td class="list_7">查看扣分：</td>
    <td class="list_8"><input type="text" name="ir.valueOfView" autocomplete="off" size="60"></td>
  </tr>
  <tr >
    <td class="list_7">注册得分：</td>
    <td class="list_8"><input type="text" name="ir.valueOfReg" autocomplete="off" size="60"></td>
  </tr>
  <tr >
    <td class="list_7">每天登录得分：</td>
    <td class="list_8"><input type="text" name="ir.valueOfLogin" autocomplete="off" size="60"></td>
  </tr>
