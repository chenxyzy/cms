...
<tr><td class="bodytitle" height=23  align="center">
 <s:action name="admin_qa_nav_findAllClass"></s:action>
    
<form method="POST" action="admin_qa_nav_order.jsp">
<table border="0" width="100%">

   <tr>
    <td width="35%" class="forumRow" align="right" >
      <p align="right">选择<s:property value="#parameters.parentID"/>父栏目：</p>
    </td>
  <center>

    <td width="65%" class="forumRow" >
      <p align="left"><select size="1" name="parentID">
				<option value="0">根栏目</option>
				<s:iterator value="#request.qaClassAll" id="x" status="st">
					<option
						<s:if test="#x.id==#parameters.parentID[0]">selected</s:if>
...
<s:action name="admin_qa_nav_findByParentId">
<s:param name="id"><s:property value="#parameters.parentID"/></s:param>
</s:action>
...
<s:iterator value="#request.qaNavAll" id="y" status="rt">

<script type="text/jscript">
   var list=new Array();
  </script>

	<s:iterator value="#request.qaNavAll" id="z" status="zt">
		<script type="text/jscript">
			list.push(<s:property value="#z.id"/>);
		</script>
...
<tr height="25" bgcolor="#FFFFFF" valign="middle" align="center" onmouseover="this.style.backgroundColor='E4E8EF'" onmouseout="this.style.backgroundColor=''"> 
    ...
    <td width="3%">${y.id}</td>
    <td width="12%" align="left">${y.title}</td>
...
<td width="12%"><s:if test="#y.state">正常</s:if><s:else><font color="#FF0000">隐藏</font></s:else></td>
    ...
    <tr>
      ...
      <td width="80%" align="center"> <s:if test="#rt.index > 0">&nbsp; <span id="666<s:property value="#y.id"/>"><a id="up<s:property value="#y.id"/>" Title="向上" href=""><b>↑</b></a></span>&nbsp;</s:if> <s:if test="#rt.index<(#request.qaNavAll.size()-1)"><a Title="向下" id="down<s:property value="#y.id"/>" href=""><b>↓</b></a></s:if>
      
<script type="text/jscript">
	$("#up<s:property value="#y.id"/>").attr("href","admin_qa_nav_swap.action?parentID=<s:property value="#pid"/>&sid=<s:property value="#y.id"/>&tid="+list[<s:property value="#rt.index"/>-1]);
	$("#down<s:property value="#y.id"/>").attr("href","admin_qa_nav_swap.action?parentID=<s:property value="#pid"/>&sid=<s:property value="#y.id"/>&tid="+list[<s:property value="#rt.index"/>+1]);
</script>
